...
class AudioPlayerSlider extends StatelessWidget {
      ...
      builder: (context, snapshot) {
        ...
        return Column(children: [
          SliderTheme(
            ...
            child: Slider(
              min: 0,
              max: track != null ? track.duration.inSeconds.toDouble() : 100,
              value: duration.inSeconds.toDouble(),
              onChanged: (value) {
                bloc.dispatch(RewindEvent(duration: Duration(seconds: value.toInt())));
              },
            ),
          ),
          Row(
  ...
  String durationToString(Duration duration) {
    ...
    if (duration.inSeconds < 10) {
      minutes = "0${(duration.inSeconds % 60).toString()}";
    } else {
      minutes = (duration.inSeconds % 60).toString();
    }
